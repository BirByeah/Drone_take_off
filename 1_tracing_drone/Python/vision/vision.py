...
class Eye:
    ...
    def image_process_routine(self) -> None:
        try:
            ...
            while not ret:
                self.logger.warning_log(f"Camera unable to open. img is {self.img}")
                continue
            #print(img_size[0] / 2)
            ...
            self.logger.warning_log(f"routine error:{e}")
        ...
        while True:
            ...
            try:
                for line in self.lines:
                    ...
                    try:
                        ...
                        self.logger.warning(f"slope:{self.rough_slope}   point:{self.rough_point}")
                        self.rough_slope[self.vis_temp_index] = 4.1 # This is a guess
                    except RuntimeWarning:
                        self.logger.warning(f"slope:{self.rough_slope}   point:{self.rough_point}")
                        self.rough_slope[self.vis_temp_index] = 4.1
                    except RuntimeError:
                        self.logger.warning(f"slope:{self.rough_slope}   point:{self.rough_point}")
                        self.rough_slope[self.vis_temp_index] = 4.1
            ...
            if abs(self.precise_slope) > 2:
                if self.start_or_end == END:
                    self.logger.debug_log(f"precise slope:{self.precise_slope}   self.start_or_end:{self.start_or_end}")
                    return
            self.adjust_itself()
    ...
    def adjust_itself(self):
        ...
        if self.start_or_end == START:
            if self.status == STATUS_FORWARD:
                self.do_wait_command(CMD_TRANSLATION, (0, step, 0, speed, 2, 70))
                task_status_list[task_index] = 0
            else:
                self.do_wait_command(CMD_TRANSLATION, (0, step, 0, speed, 0, 90))
                task_status_list[task_index] = 0
        else:
            try:
            ...
            if img_size[0] / 4 >= rough_center[1]:
                ...
                if ADVANCE_MODE:
                    ...
                    self.do_wait_command(CMD_TRANSLATION, (0, 5, 0, speed, 0, 0))
                    task_status_list[task_index] = 0
